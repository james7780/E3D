...
//                           for Windows 32 / OpenGL environment
//
...
void E3D_PlatformSpecific::RenderScene(E3D_Scene *scene, float viewWidth, float viewHeight, bool clear)
{
	...
	gluPerspective(camera->GetFOV(), viewWidth/viewHeight, 1.0f, 1000.0f);
	...
	for(int i=0; i<scene->GetNumObjects(); i++) {
		...
		if(dist < scene->GetRenderDistance()) {
			...
			if(pO->parent) {
				Vector p = pO->parent->position;
				glTranslatef(p.x, p.y, p.z);
				Vector r = pO->parent->rotation;
				...
				Vector s = pO->parent->scale;
				glScalef(s.x, s.y, s.z);
			}

			glTranslatef(pO->position.x, pO->position.y, pO->position.z);
			glRotatef(RAD2DEG(pO->rotation.y) , 0.0f, 1.0f, 0.0f);			// Spin Left And Right
			glRotatef(RAD2DEG(pO->rotation.z), 0.0f, 0.0f, 1.0f);			// roll
			glRotatef(RAD2DEG(pO->rotation.x), 1.0f, 0.0f, 0.0f);			// pitch
			glScalef(pO->scale.x, pO->scale.y, pO->scale.z);
